...
import { Bot, User, BrainCircuit, Lock, Clapperboard, ShieldAlert, Gavel, BookOpen, CheckCircle2, Circle, Microscope, ChevronDown, ChevronUp, Clock, Smile, Activity, MapPin, Eye, EyeOff, ArrowRight, Globe, Link as LinkIcon, ExternalLink, Copy, Check } from 'lucide-react';
...
const parseMessageContent = (text: string): Token[] => {
  ...
  codeBlockSplit.forEach((segment, index) => {
      ...
      // 4. Advanced Splitting (Prioritize LaTeX over other tokens)
      // Groups: 1=BlockMath, 2=InlineMath, 3=Thought, 4=Whisper, 5=Action
      // NOTE: Regex for InlineMath `\$[^$]*?\$` prevents greedy matching.
      const compositeRegex = /(\$\$[\s\S]*?\$\$)|(\$[^$]*?\$)|(\[.*?\])|(\{.*?\})|((?<!https?:)\/\/.*?\/\/)/g;
      
      let lastIndex = 0;
      let match;
      
      while ((match = compositeRegex.exec(contentToProcess)) !== null) {
          // Add preceding text
          if (match.index > lastIndex) {
              const text = contentToProcess.slice(lastIndex, match.index);
              if (text.trim()) tokens.push({ type: 'text', content: text });
          }

          if (match[1] || match[2]) {
              // LaTeX (Block or Inline) -> Treat as TEXT to let RemarkMath handle it
              tokens.push({ type: 'text', content: match[0] });
          } else if (match[3]) {
              // Thought [ ... ]
              tokens.push({ type: 'thought', content: match[3].slice(1, -1) });
          } else if (match[4]) {
              // Whisper { ... }
              tokens.push({ type: 'whisper', content: match[4].slice(1, -1) });
          } else if (match[5]) {
              // Action // ... //
              tokens.push({ type: 'action', content: match[5].slice(2, -2) });
          }
      ...
      if (lastIndex < contentToProcess.length) {
          const text = contentToProcess.slice(lastIndex);
          if (text.trim()) tokens.push({ type: 'text', content: text });
      }
  });
...
const CopyAction = ({ text, className = "", iconColor }: { text: string, className?: string, iconColor?: string }) => {
    ...
    return (
        <button 
            onClick={handleCopy} 
            className={`p-1.5 rounded-md hover:bg-black/10 dark:hover:bg-white/10 transition-all ${className}`}
            title="复制内容"
        >
            {copied ? <Check size={12} className="text-green-500" /> : <Copy size={12} className={iconColor || "text-slate-400"} />}
        </button>
    )
};
...
const ChatMessage: React.FC<ChatMessageProps> = ({ 
  ...
  const handleTouchStart = () => {
    ...
    if (!selectionMode) {
      timerRef.current = window.setTimeout(() => {
        // If not scrolling, AND no text selected, trigger long press
        const selection = window.getSelection();
        const hasTextSelected = selection && selection.toString().length > 0;
        
        if (!isScrollingRef.current && !hasTextSelected) {
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    // If text is selected, allow the default browser context menu (Copy, etc.)
    const selection = window.getSelection();
    if (selection && selection.toString().length > 0) {
        return; 
    }

    // Otherwise, trigger custom behavior (Multi-select)
    if (!selectionMode) {
        e.preventDefault();
        onLongPress(message.id);
    }
  ...
  return (
    <div 
      ...
      className={`
        ...
        ${selectionMode ? 'select-none cursor-pointer' : 'select-text'}
      `}
    ...
    >
      ...
      `}>
        ...
        <div className="flex-shrink-0 mt-1 flex flex-col items-center gap-1 select-none">
          {isUser ? (
        ...
        <div className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} min-w-0 flex-1 max-w-full`}>
          {!isUser && (
            <div className="flex items-center gap-2 mb-1 ml-1 flex-wrap select-none">
             ...
             {message.isError ? (
               ...
               tokens.map((token, index) => {
                 if (token.type === 'social_block') {
                    ...
                    const socialContent = safeStr(m['Language']);
                    return (
                        <div key={index} className="bg-white dark:bg-[#151516] border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-xl w-full max-w-2xl mb-2 relative overflow-hidden group/card hover:shadow-2xl transition-all">
                            ...
                            <div className="absolute top-0 right-0 p-3 opacity-10 group-hover/card:opacity-20 transition-opacity pointer-events-none select-none">
                                <Activity size={80} />
                            </div>
                            
                            {/* Copy Button */}
                            <div className="absolute top-4 right-4 z-10">
                                <CopyAction text={socialContent} />
                            </div>
                            
                            <div className="flex items-center gap-2 mb-4 text-xs font-mono text-slate-400 select-none">
                                <Clock size={12} />
                   ...
                   return (
                     <div key={index} className={`
                        relative px-4 py-2.5 md:px-6 md:py-3.5 shadow-sm text-sm md:text-[15px] leading-relaxed max-w-full group/text
                        ${bubbleStyle}
                     `}>
                        ...
                        <div className="absolute -bottom-6 right-0 md:bottom-1 md:right-1 z-10 opacity-0 group-hover/text:opacity-100 transition-opacity">
                             <CopyAction text={token.content} iconColor={isUser ? "text-slate-300 hover:text-white" : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"} />
                        </div>
                     </div>
                   );
                 } else if (token.type === 'thought') {
                   return (
                     <div key={index} className="relative max-w-full bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 text-xs md:text-sm px-3 py-2 md:px-4 md:py-2 rounded-2xl italic flex items-start gap-2 backdrop-blur-sm group/thought">
                       <BrainCircuit size={14} className="mt-1 shrink-0 opacity-70 select-none" />
                       ...
                       <div className="opacity-0 group-hover/thought:opacity-100 transition-opacity">
                           <CopyAction text={token.content} />
                       </div>
                     </div>
                   );
                 } else if (token.type === 'whisper') {
                   return (
                     <div key={index} className="relative max-w-full bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800/30 text-purple-800 dark:text-purple-300 text-xs md:text-sm px-3 py-2 md:px-4 md:py-2 rounded-2xl flex items-start gap-2 shadow-inner group/whisper">
                       <Lock size={14} className="mt-1 shrink-0 opacity-70 select-none" />
                       <div className="w-full overflow-hidden">
                         <div className="text-[10px] font-bold uppercase tracking-wider opacity-50 mb-0.5 select-none">Secret</div>
                       ...
                       <div className="opacity-0 group-hover/whisper:opacity-100 transition-opacity">
                           <CopyAction text={token.content} />
                       </div>
                     </div>
                   );
                   ...
                   return (
                     <div key={index} className="inline-flex items-center gap-2 text-amber-600 dark:text-amber-400 text-xs md:text-sm font-bold italic px-2 py-1 max-w-full overflow-hidden select-none">
          ...
          {hasGrounding && showSources && (
             <div className="w-full max-w-full mt-2 bg-emerald-50 dark:bg-[#0d211c] border border-emerald-100 dark:border-emerald-900 rounded-xl overflow-hidden animate-slide-up select-none">
