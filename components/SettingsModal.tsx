    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
      ...
      <div className="bg-white dark:bg-[#1e1e1e] w-full max-w-5xl h-[95dvh] sm:h-auto sm:max-h-[90vh] rounded-t-[2rem] sm:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden ring-1 ring-slate-200 dark:ring-slate-700 transition-all animate-slide-up">
        ...
        <div className="px-6 py-4 sm:px-8 sm:py-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-[#1e1e1e] shrink-0 z-10 sticky top-0">
          ...
          <button
            onClick={onClose}
            className="p-3 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-400 hover:text-slate-600 active:scale-95 touch-manipulation"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-[#f8fafe] dark:bg-[#121212] overscroll-contain pb-24 sm:pb-6">
            ...
            {onLoadCloudConfig && (
              <>
                <button
                  ...
                  className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-4 sm:py-3 rounded-2xl bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 transition-all text-sm font-bold text-white shadow-lg shadow-blue-500/20 active:scale-95 touch-manipulation"
                >
                  <Cloud size={18} />
                  云端下载
                </button>
                <button
                  ...
                  className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-4 sm:py-3 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 transition-all text-sm font-bold text-white shadow-lg shadow-emerald-500/20 active:scale-95 touch-manipulation"
                >
                  <CloudUpload size={18} />
                  云端备份
                </button>
          ...
          {/* --- CONTEXT COMPRESSION SECTION (OPTIMIZED UI) --- */}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 relative z-10 gap-4">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-lg leading-relaxed">
                  当对话过长时，自动将早期记忆压缩为摘要，大幅降低 Token 消耗并加速响应。
                </p>
              </div>

              <div className="flex items-center gap-3 self-end sm:self-auto">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">{contextConfig.enableCompression ? '已开启' : '已关闭'}</span>
                <label className="relative inline-flex items-center cursor-pointer touch-manipulation p-2 -m-2">
                  <input
                    type="checkbox"
                ...
                <div className="flex flex-col gap-4">
                  <div className="flex justify-between items-center">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">
                      保留最近消息数 (Sliding Window)
                    </label>
                    <span className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-300 px-3 py-1 rounded-lg text-xs font-bold border border-indigo-100 dark:border-indigo-800">
                      {contextConfig.maxHistoryMessages} 条
                    </span>
                  </div>
                  ...
                  <div className="relative py-2">
                    <input
                      ...
                      className="w-full h-4 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-600 touch-pan-x"
                    />
                  ...
                  <div className="p-3 bg-indigo-50/50 dark:bg-indigo-900/10 rounded-xl text-[11px] text-indigo-800 dark:text-indigo-300 leading-relaxed border border-indigo-100 dark:border-indigo-900/30">
                    <Sparkles size={12} className="inline mr-1" />
                    系统将始终保留最近 {contextConfig.maxHistoryMessages} 条消息的原始内容。更早的消息将被“皇家档案员”整理归档，尽可能保留详细剧情。
                  </div>
                </div>
              </div>
            )}
      ...
      {/* Cloud Config Selector Modal */}
      {onLoadCloudConfig && (
        <CloudConfigSelector
          isOpen={showCloudSelector}
      ...
      {/* Cloud Sync Modal for Backup/Upload */}
      {showCloudSync && (
        <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg max-h-[80vh] overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center">
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">云端配置管理</h3>
              <button
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              <CloudSync
                ...
                onLoadConfig={(config) => {
                  if (onLoadCloudConfig) {
                    onLoadCloudConfig(config);
                  }
                  setShowCloudSync(false);
                }}
                configType="participant"
              />
            </div>
          </div>
        </div>
