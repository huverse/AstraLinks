...
const MultimodalCenter: React.FC<MultimodalCenterProps> = ({ isOpen, onClose, participants, isTokenLimitExceeded, onTokenLimitExceeded }) => {
...
只返回 JSON，不要其他解释。`;
    ...
    const handleAction = async () => {
        ...
        if (isTokenLimitExceeded && onTokenLimitExceeded) {
                                        ...
                                        <div>
                                            ...
                                            <div className="flex justify-between text-[10px] text-slate-500 mt-1">
                                                ...
                                                <span className="text-amber-500">
                                                    {(() => {
                                                        const model = (currentTab.customModel || globalConfig.modelName || '').toLowerCase();
                                                        if (model.includes('veo-3') || model.includes('veo3')) return '推荐: 4-8秒 (Veo 3)';
                                                        if (model.includes('veo-2') || model.includes('veo2')) return '推荐: 5-8秒 (Veo 2)';
                                                        if (model.includes('veo')) return '推荐: 4-8秒 (Veo)';
                                                        if (model.includes('runway')) return '推荐: 4-16秒 (Runway)';
                                                        if (model.includes('pika')) return '推荐: 3-4秒 (Pika)';
                                                        if (model.includes('sora')) return '推荐: 5-20秒 (Sora)';
                                                        return '自定义时长';
                                                    })()}
                                                </span>
                                                <span>60秒</span>
    ...
    return (
        <>
            ...
            {/* Cloud Sync Modal for Multimodal Config */}
            {showCloudSync && (
                    ...
                    <div className="bg-[#1c1c1e] w-full max-w-lg rounded-2xl border border-white/10 shadow-2xl">
                        <div className="p-6 border-b border-white/10 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                <Cloud size={20} className="text-blue-400" /> 多模态云端配置
                            </h3>
                            <button
