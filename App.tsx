...
const ThinkingIndicator = ({ participantName }: { participantName?: string }) => (
  <div className="flex items-center gap-3 p-4 animate-fade-in w-full justify-start">
    ...
    <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
      ...
      <span className="text-xs font-bold">
        {participantName ? `${participantName} 正在思考...` : "AI 正在思考..."}
      </span>
    </div>
  </div>
);
...
const App: React.FC = () => {
  ...
  const [showKickConfirm, setShowKickConfirm] = useState<{ targetId: string, reason: string } | null>(null);
  ...
  const triggerBackgroundCompression = async (targetSessionId: string, msgs: Message[]) => {
    ...
    // OPTIMIZATION: Increase buffer to 20 messages. 
    // This means we wait until we have [Window Size] + 20 new messages before triggering compression.
    // This reduces frequency of rewriting, thus preserving quality.
    const threshold = s.contextConfig.maxHistoryMessages + 20;
  ...
  const handleResetTokenUsage = (id: string) => {
    if (!isAuthenticated) {
      alert('请先登录后才能重置 Token 统计数据。\n\nPlease login to reset token usage.');
      setIsLoginOpen(true);
      return;
    }
  ...
  // Token limit for unauthenticated users
  const FREE_TOKEN_LIMIT = 10000;
  const getTotalTokenUsage = () => {
    return participants.reduce((sum, p) => sum + (p.tokenUsage?.totalTokens || 0), 0);
  };
  ...
  const handleKickConfirm = () => {
    if (showKickConfirm) {
      const targetId = showKickConfirm.targetId;
      const reason = showKickConfirm.reason;

      // Execute Kick (Disable config)
      handleUpdateParticipant(targetId, { enabled: false });

      const targetName = participantsRef.current.find(p => p.id === targetId)?.nickname ||
        participantsRef.current.find(p => p.id === targetId)?.name || 'Unknown';

      const kickMsg: Message = {
        id: Date.now().toString(),
        senderId: 'SYSTEM',
        content: `**[系统公告]**: 玩家 ${targetName} 已确认被移出配置 (Disabled). 原因: ${reason}`,
        timestamp: Date.now()
      };
      ...
      updateSessionById(activeSessionId, {
        ...
        pendingKickRequest: null,
        isProcessing: false // Ensure stopped
      });
  ...
  const handleKickCancel = () => {
    setShowKickConfirm(null);
    // Resume or just stop? Best to just stop and let user decide next move to avoid weird states.
    updateSessionById(activeSessionId, { isProcessing: false });
  };
  ...
  const processPartyRound = async (targetSessionId: string, history: Message[], specificParticipantIds?: string[], forceTriggerJudge: boolean = false) => {
    ...
    // Helper: Deduplication Check (Look back 5 messages)
    const appendMessages = (newMsgs: Message[]) => {
      const uniqueMsgs = newMsgs.filter(m => {
        // Check against last 5 messages to be safe
        const recentHistory = currentRoundHistory.slice(-5);
        const isDuplicate = recentHistory.some(prev =>
          prev.senderId === m.senderId &&
          prev.content === m.content &&
          Math.abs(prev.timestamp - m.timestamp) < 5000 // Within 5 seconds
        );
        return !isDuplicate;
      });
    ...
    try {
      ...
      if (isJudgeMode && specialRoleParticipant && specialRoleParticipant.config.apiKey) {
        ...
        if (parsedRef.kickRequest) {
          ...
          if (target) {
            // Append the Referee's message (e.g. "I am kicking...")
            appendMessages(parsedRef.messages);
            updateSessionById(targetSessionId, {
              messages: currentRoundHistory,
              isProcessing: false // STOP processing to wait for confirmation
            });

            setShowKickConfirm({ targetId: target.id, reason: parsedRef.kickRequest.reason });
        ...
        if (parsedRef.nextSpeakers.length > 0) {
          ...
          // If Judge doesn't say "NEXT", we shouldn't assume everyone speaks.
          // We wait for user input unless status is strictly ACTIVE.
          // BUT: If user explicitly triggered forceTriggerJudge (like manual call), we might just stop.
          nextIds = [];
        }
        ...
        if (playersActed && !signal.aborted) {
          ...
          // DUPLICATE CHECK: Use robust check
          ...
          // RECURSION CHECK: Only recurse if Referee designates NEXT speakers explicitly.
          // And ensure we check signal and stopped state inside timeout.
          if (parsedResolve.nextSpeakers.length > 0 && !parsedResolve.nextSpeakers.includes('NONE')) {
            setTimeout(() => {
              // CRITICAL: Re-check stop/abort conditions inside the closure
        ...
        for (const p of activePlayers) {
          ...
          const newMessage: Message = {
            ...
            // Add Agent Commands Execution
            agentCommands: response.agentCommands // Pass commands
          };
  ...
  const executeAgentCommands = (commands: any[], callerId?: string) => {
    commands.forEach(cmd => {
      ...
      if (cmd.tool === 'browser_open' && cmd.args?.url) {
        window.open(cmd.args.url, '_blank');
      }
      else if (cmd.tool === 'ui_theme_control') {
        // Assuming tailwind dark mode class on html/body
        if (cmd.args?.mode === 'dark') document.documentElement.classList.add('dark');
        else if (cmd.args?.mode === 'light') document.documentElement.classList.remove('dark');
      }
      else if (cmd.tool === 'ui_navigate') {
        if (cmd.args?.target === 'settings') setIsSettingsOpen(true);
        else if (cmd.args?.target === 'multimodal') setIsMultimodalOpen(true);
        else if (cmd.args?.target === 'collaboration') setIsCollaborationOpen(true);
      }
      else if (cmd.tool === 'app_mode_switch') {
        // Map text to enum
        const map: Record<string, GameMode> = {
          'FREE_CHAT': GameMode.FREE_CHAT,
          'JUDGE_MODE': GameMode.JUDGE_MODE,
          'NARRATOR_MODE': GameMode.NARRATOR_MODE
        };
        if (cmd.args?.mode && map[cmd.args.mode]) {
  ...
  const handleSend = async () => {
    ...
    // Token limit check for unauthenticated users
    if (!isAuthenticated) {
      const totalUsed = getTotalTokenUsage();
      if (totalUsed >= FREE_TOKEN_LIMIT) {
        alert(`免费 Token 已用完 (${totalUsed.toLocaleString()}/${FREE_TOKEN_LIMIT.toLocaleString()})\n\n请登录以继续使用所有功能。\nFree tokens exhausted. Please login to continue.`);
        setIsLoginOpen(true);
        return;
      }
    }
  ...
  const handleDeleteSelected = (e?: React.MouseEvent) => {
    ...
    // Use setTimeout to ensure the event loop completes before showing confirm
    setTimeout(() => {
      if (window.confirm(`确定删除 ${selectedMsgIds.size} 条消息吗？`)) {
        updateActiveSession({ messages: activeSession.messages.filter(m => !selectedMsgIds.has(m.id)) });
        exitSelectionMode();
      }
    }, 10);
  };
  ...
  return (
    <div className="flex h-[100dvh] bg-[#f5f5f7] dark:bg-black font-sans text-slate-900 dark:text-slate-100 overflow-hidden relative selection:bg-blue-200 dark:selection:bg-blue-900">
      ...
      {/* --- Sidebar --- */}
      <div className={`
        fixed lg:static inset-y-0 left-0 w-80 bg-[#f5f5f7] dark:bg-[#1c1c1e] border-r border-slate-200 dark:border-black/50 p-4 z-[50] transform transition-transform duration-300 ease-in-out flex flex-col
      ...
      <div className="flex-1 flex flex-col relative bg-white dark:bg-black h-full w-full max-w-full">
        ...
        <div className="h-16 bg-[#f5f5f7]/80 dark:bg-[#1c1c1e]/80 backdrop-blur-md border-b border-slate-200 dark:border-black/50 flex items-center justify-between px-4 z-20 sticky top-0 shrink-0">
          ...
          <div className="flex gap-2">
            ...
            >
              <RefreshCw size={42} className={activeSession.isAutoLoop ? 'animate-spin-slow' : ''} />
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 scroll-smooth relative" ref={chatContainerRef}>
          ...
          {activeSession.isProcessing && (
            <div className="w-full flex justify-start">
              <ThinkingIndicator
                participantName={participants.find(p => p.id === activeSession.currentTurnParticipantId)?.nickname || participants.find(p => p.id === activeSession.currentTurnParticipantId)?.name}
              />
            </div>
          )}
          ...
          <div className="flex items-end gap-3 max-w-4xl mx-auto">
            ...
            <div className="flex-1 bg-slate-100 dark:bg-[#1c1c1e] rounded-[1.5rem] flex items-center px-4 py-2 border border-transparent focus-within:border-blue-500/50 focus-within:bg-white dark:focus-within:bg-black transition-all shadow-inner">
              <textarea
                ...
                placeholder={
                  activeSession.isProcessing ? "AI 正在思考中... (点击右侧按钮停止)" :
                    isJudgeModeActive ? "输入行动，或 @裁判 寻求介入..." :
                      "输入消息..."
                }
                // Allow typing even if processing, but Enter key might be blocked by logic. 
                // Better to keep disabled to prevent confusion, or allow queuing? 
                // For now, let's keep disabled to strictly force Stop.
                disabled={activeSession.isProcessing}
                rows={1}
                className="w-full bg-transparent border-none outline-none text-slate-800 dark:text-slate-100 placeholder:text-slate-400 resize-none max-h-[150px] py-2 leading-relaxed disabled:opacity-50"
              />
            </div>

            <button
              onClick={activeSession.isProcessing ? handleStop : handleSend}
              // CHANGE: Do NOT disable if processing. Only disable if idle AND empty input.
              disabled={!activeSession.isProcessing && (!inputText.trim() && inputImages.length === 0)}
              className={`
                 p-3 rounded-2xl transition-all shadow-lg active:scale-95 shrink-0 flex items-center justify-center
                 ${activeSession.isProcessing
                  ? 'bg-red-500 hover:bg-red-600 text-white shadow-red-500/30' // Stop Button Style
                  : (!inputText.trim() && inputImages.length === 0)
                    ? 'bg-slate-200 dark:bg-white/10 text-slate-400 cursor-not-allowed'
                    : 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-500/30'
                }
               `}
              title={activeSession.isProcessing ? "停止生成 / Stop Generation" : "发送消息"}
            >
              {activeSession.isProcessing ? (
      ...
      <MultimodalCenter
        ...
        participants={participants}
        isTokenLimitExceeded={!isAuthenticated && getTotalTokenUsage() >= FREE_TOKEN_LIMIT}
        onTokenLimitExceeded={() => {
          alert(`免费 Token 已用完 (${getTotalTokenUsage().toLocaleString()}/${FREE_TOKEN_LIMIT.toLocaleString()})\n\n请登录以继续使用所有功能。\nFree tokens exhausted. Please login to continue.`);
          setIsLoginOpen(true);
        }}
      />
      ...
      {showKickConfirm && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
          <div className="bg-white dark:bg-[#1e1e1e] w-full max-w-sm p-6 rounded-3xl shadow-2xl border border-red-500/30 animate-slide-up">
            <div className="flex flex-col items-center text-center mb-6">
              <div className="w-20 h-20 bg-red-100 dark:bg-red-900/30 rounded-full flex items-center justify-center text-red-600 mb-4 animate-pulse">
                <UserX size={40} />
              </div>
              <h3 className="text-xl font-bold text-red-600 dark:text-red-400">确认踢出玩家？</h3>
              <p className="text-sm font-medium text-slate-700 dark:text-slate-200 mt-2">
                裁判判定将以下玩家移出游戏：
              </p>
              <div className="mt-4 p-3 bg-slate-100 dark:bg-black/30 rounded-xl w-full text-left border border-slate-200 dark:border-slate-800">
                <div className="text-xs font-bold text-slate-500 uppercase">Target</div>
                <div className="font-bold text-slate-800 dark:text-white truncate">
                  {participants.find(p => p.id === showKickConfirm.targetId)?.nickname ||
                    participants.find(p => p.id === showKickConfirm.targetId)?.name || showKickConfirm.targetId}
                </div>
                <div className="text-xs font-bold text-slate-500 uppercase mt-2">Reason</div>
                <div className="text-sm text-slate-600 dark:text-slate-400 italic">{showKickConfirm.reason}</div>
              </div>
              <p className="text-xs text-slate-400 mt-4">
                这将会在全局配置中禁用该模型，使其无法继续发言。
              </p>
            </div>
            <div className="flex gap-3">
              <button onClick={handleKickCancel} className="flex-1 py-3 rounded-xl bg-slate-100 dark:bg-white/10 font-bold text-sm text-slate-600 dark:text-slate-300">取消</button>
              <button onClick={handleKickConfirm} className="flex-1 py-3 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-sm shadow-lg shadow-red-500/30">
                确认踢出
              </button>
            </div>
          </div>
